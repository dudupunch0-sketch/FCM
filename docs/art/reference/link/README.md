...
- 현재 이미지들은 **공식 홍보 일러스트**이며 게임 화면이나 모델 정투영 도면이 아니다. 붓 터치까지 게임 셰이더의 특성으로 단정하지 않는다.
- 정확한 셰이딩과 입체 비례 확정에는 실제 게임의 얼굴 근접 화면과 정면·측면 자료가 추가로 필요하다. 이번 묶음에 그 자료가 있다고 주장하지 않는다.
