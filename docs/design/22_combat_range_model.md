...
각도 이점의 보상은 데미지 배수가 아니라 **가드가 헛도는 것**이다.
가드는 방향을 덮는다. 정면을 잃은 선수는 펀치가 더 이상 오지 않는 쪽을 막고 있다.

`angle.guardLeak`이 그 계수다. 이것이 없을 때 각도 이점은 사실상 데미지 +20%였고,
셸을 두른 상대는 각을 내주고도 **모든 펀치를 막았다.** 실측으로 세 방의 합계 위력이
10.0에서 21.9로 올랐다.

**다만 가드가 무의미해지지는 않는다.** 검증기가 가장 성긴 가드조차
`blockLeak × guardLeak < 1`을 지키도록 강제한다. 그렇지 않으면
사이드 스텝의 답이 "가드를 그만두는 것"이 된다.

## 사이드 스텝은 사용률로 재지 않는다

가장 비싼 회피이고, 읽기가 틀리면 응징당하며, 창은 두 행동뿐이다.
**균형에서 드물게 나오는 것이 정상이다.**

재야 할 것은 점유율이 아니라 **읽기가 맞았을 때의 전환율**이다.
지치고 가드만 하는 상대를 각을 잡고 몰아쳐 끝낼 수 있는가가 이 카드의 시험이다.
...
- 각도 이점의 지속 행동 수와 계수 실측. 현재 2행동, `guardLeak` 2.0
...
- 각을 잡으면 가드를 뚫고 들어가되, 가드가 무방비보다는 여전히 낫다
