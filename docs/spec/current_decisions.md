...
## 정보 카드와 스타일 카드의 한도 공유
세부 기준: `docs/design/31_information_economy_and_placement.md`
실물 Config: `config/information_cards.json`

- **정보 카드 전용 슬롯을 만들지 않는다.** 전용 슬롯은 항상 비어 있으므로 항상 채우게 되고, 결국 정보 카드를 구조적으로 필수화한다. `18번`이 막으려던 상태를 설계로 보장하는 셈이다.
- 같은 5장 한도 안에서 경쟁시키되 **무카드 기준선을 실제로 쓸 만하게 유지한다.** 정보 카드가 파는 것은 정보가 아니라 **확실성**이다. 기준선은 밸런스 항목이 아니라 시스템 성립의 전제이며 끌 수 없다.
- 카드별 공개량 대신 **턴당 총 공개 예산**을 둔다. 장착한 모든 정보 카드가 하나의 예산을 나눠 쓰므로 중첩 수익이 체감한다. 두 번째 카드는 더 많은 정보가 아니라 더 넓은 발동 조건을 산다.
- **최대 예산에서도 8칸 계획 전체를 볼 수 없다.** 부분적 지식은 구조이지 튜닝 결과가 아니다.
- 카드 성장은 정밀도와 발동 조건을 개선하고 **예산 상한은 올리지 않는다.** 성장이 전체 공개로 수렴하는 경로를 막는다.
