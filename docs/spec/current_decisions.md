...
## 정보 공개형과 스타일형의 한도 분리
세부 기준: `docs/design/31_information_economy_and_placement.md`
실물 Config: `config/information_cards.json`

정보 카드는 별개의 카드 종류가 아니라 **Skill Card의 한 갈래**다. Action Card(타임라인 배치)와 구분한다.
정보 공개형도 선수의 특성이다. 상대를 읽어내는 눈이지 플레이어의 외부 도구가 아니다.

- **활성 한도를 갈래별로 나눈다.** 스타일형 5장(기존 유지), 정보 공개형 3장.
- **정보 3장에서는 카드 한도가 아니라 공개 예산이 실질 상한이다.** 세 장을 다 끼워도 한 턴에 모두 최고 정밀도로 발동할 수 없다. 추가 카드는 한 턴의 정보량이 아니라 **여러 상황에 걸친 발동 조건 커버리지**를 산다. 예산을 카드 수에 비례시키면 이 구조가 무너진다.
- 분리는 **정보 카드가 사실상 항상 장착됨을 받아들인다.** 대신 플레이어의 결정이 "확실성이냐 위력이냐"(비교 불가)에서 **"어떤 눈을 가져갈 것이냐"**(비교 가능)로 바뀌고, 양쪽 콘텐츠가 모두 실제로 쓰여 밸런싱 근거가 생긴다.
- **되돌리는 조건을 명시한다.** ① 정보 카드가 계획을 시시하게 만듦 ② 한 카드가 나머지를 압도 ③ 전용 한도를 줬는데도 스타일 카드가 안 쓰임. 해당하면 단일 한도로 통합한다.
- 한도와 별개로 **턴당 총 공개 예산**을 유지한다. 카드 하나가 8칸을 다 공개하는 것을 막고, 나중에 정보 한도를 올릴 때 중첩 규칙을 새로 만들 필요가 없다.
- **최대 예산에서도 8칸 계획 전체를 볼 수 없다.** 부분적 지식은 구조이지 튜닝 결과가 아니다.
- 카드 성장은 정밀도와 발동 조건을 개선하고 **예산 상한은 올리지 않는다.**
- 공개 경쟁은 발동 조건 구체성 → 레벨 → ID 순으로 결정론적으로 해소한다. 같은 배치를 두 카드가 공개하면 예산은 한 번만 쓴다.
- **무카드 기준선은 유지하며 품질이 Fight IQ에 따라 달라진다.** 눈이 좋은 선수는 기준선이 이미 넓고 정확해 카드 이득이 작고, 둔한 선수는 카드로 메꾼다. 밸런스 장치가 아니라 특성 설정의 당연한 결과이며 `28번`의 Fight IQ 값을 그대로 쓴다.
- NPC는 이 예산 체계를 쓰지 않는다. 회고적으로만 작동한다(`28번`).

## 배치 규칙
빈칸은 호흡 정리로 처리한다. 동일 동작 반복은 허용한다 — 반복은 패턴 노출을 키워 **이미 게임이 처벌하므로** 별도 금지 규칙을 두지 않는다.
8칸을 넘기는 삽입은 뒷부분을 조용히 잘라내지 않고 거부한다. 확정한 계획과 실행 계획이 달라지면 안 된다.
...
- **스태미너에는 내려가기만 하는 회복 상한이 있다.** 세부 기준: `docs/design/38_stamina_attrition.md`. 호흡 카드·턴 사이 회복·라운드 인터벌 세 경로 모두가 이 상한에서 잘리므로, 경기 중 깎인 것은 경기가 끝날 때까지 돌아오지 않는다. 상한이 없으면 소모는 언제나 일시적이고 소모전이라는 축이 존재하지 않는다. 실측에서 스태미너 중앙값이 100이었고 방어·바디방어 역할이 균형에서 한 번도 선택되지 않았다.
- 상한을 깎는 것은 **지친 채로 공격한 것**과 **바디 타격을 받은 것** 둘뿐이다. 머리는 손상과 KO의 축, 바디는 소모의 축이라는 구분을 유지한다. 지쳤는지는 절대값이 아니라 **자기 상한 대비 비율**로 본다.
- 상한 저하에는 **바닥이 있어야 한다.** 저하는 자기를 먹이는 되먹임이므로 바닥이 없으면 한 번 밀린 선수가 회복 불가능한 나선에 들어간다. 닳은 선수는 위험해야지 무력해서는 안 된다. 검증기가 바닥이 바디 KO 문턱보다 낮아지는 것을 거부한다.
- 지치면 **때리는 힘이 줄고 맞는 힘이 는다.** 둘 다 절대 스태미너를 읽으므로 상한이 내려간 선수는 별도 규칙 없이 더 취약해진다.
- **위력 편차 난수는 행위자별로 뽑지 않는다.** 같은 칸의 두 타격은 같은 순간이므로 편차를 공유한다. 행위자를 시드에 넣으면 거울 대결이 정확히 대칭이 아니게 되고, 페이오프 행렬이 반대칭을 강제하므로 "게임 값 0" 검사가 좌우 비대칭을 잡지 못한다.
...
- **사이드 스텝은 좌우 방향을 가지며, 그 선택이 카드의 전부다.** 세부 기준: `docs/design/22_combat_range_model.md`. 직선은 어느 쪽으로 빠지든 빗나가지만, 훅은 감아 들어오므로 **돌아 나간 쪽의 손**만 따라온다. 반대쪽 손 훅은 지나간다. 따라서 범용 회피가 아니라 **어느 손이 오는가에 대한 읽기**다. 양쪽 훅을 모두 피하는 사이드 스텝은 선택이 아니라 정답이 되므로 금지하며, 한쪽 손에만 훅이 있어 반대쪽 스텝이 위험 없이 안전해지는 것도 금지한다. 검증기가 둘 다 강제한다.
- **사이드 스텝의 방향은 내 움직임의 물리적 좌우로 선언하고, 어느 손이 어느 쪽에서 나오는지는 던지는 쪽의 살아있는 스탠스로 푼다.** 상대 기준 lead/rear로 쓰면 내 움직임을 상대의 스탠스 용어로 정의하는 것이 되어, 상대가 콤보 중간에 스위치하면 이미 커밋된 내 스텝이 소급해서 뒤집힌다. 상대가 스탠스를 바꿔도 내가 순간이동하지는 않는다. 상대의 왼팔은 스위치해도 왼팔이며, 바뀌는 것은 그 팔이 앞손이냐 뒷손이냐뿐이다.
- **Stance는 선수 스펙이 아니라 살아있는 전투 상태다.** 경기 중 `stance` 종류 카드로 바뀌며, 바뀐 뒤에는 내 훅들이 나오는 쪽이 반대가 된다. 손상 좌우 해석도 같은 살아있는 값을 읽는다.
- **각도 이점의 노출은 칸이 아니라 상대의 행동 수로 센다.** 이미 저지른 행동 + 한 행동 뒤에는 카드를 쓰지 않고 스스로 정면을 되찾는다. 칸으로 세면 1칸 잽과 4칸 셸이 같은 노출을 치러 방향이 거꾸로다 — 긴 동작이야말로 더 오래 정면을 잃어야 한다.
- 스텝하지 못한 경우(휘청임·페이크 피격 등)는 스텝 응징을 받지 않고 평범한 빗나간 회피로 처리한다. 응징은 **실제로 돌아 들어간 쪽**에만 붙는다.
...
- **계산 가능한 것은 저장하지 않는다.** Derived와 Effective는 세이브에 넣지 않고 로드 시 Definition Data로 재계산한다. 저장하면 밸런스 패치와 기존 세이브가 어긋난다.
- `save_version`(스키마)과 `config_version`(밸런스 데이터)은 정책이 다르다. 스키마 불일치는 전진 마이그레이션 필수, 밸런스 불일치는 경고 후 로드 허용. 패치로 수치가 달라지는 것은 정상이다.
- 마이그레이션은 한 단계씩 누적한다. 직행 경로를 만들지 않아 테스트 경로를 하나로 유지한다.
- Definition 삭제보다 **비활성화를 기본**으로 한다. 끊어진 참조는 로드 시 검출하고 기록한다.
- **Seed만으로는 재현되지 않는다.** 스트림별 소비 카운터를 저장하고 RNG 스트림을 용도별로 분리한다. 전투 코드 수정이 월드 생성 결과를 흔들면 밸런스 비교가 불가능해진다.
- 전투 중 저장은 전투 상태 전체가 아니라 **시작 스냅샷 + 확정 계획 시퀀스**로 한다. 리플레이와 같은 메커니즘을 쓰며, 복원 불일치 자체가 결정론이 깨졌다는 신호가 된다.
- Event History는 압축하되 **Evidence 출처 이벤트는 축약하지 않는다.** 축약하면 `29번`의 과거 자료 재해석이 불가능해진다.
...
## 난이도
세부 기준: `docs/design/33_difficulty.md`
실물 Config: `config/difficulty.json`

밸런스 기준선이 측정되면서 선행 조건이 충족되어 유예에서 해제했다.

- **난이도는 마주하는 세계와 그것을 보는 선명도를 바꾼다.** 상대 수준, 정보 품질, 경제 압박, 유망주 공급량이 대상이다.
- **전투 판정·난수·위력 공식·상태 임계·판정 집계는 조절하지 않는다.** Config의 금지 목록으로 명시하고 로더가 삭제를 거부한다. 같은 선수·계획·Seed는 모든 등급에서 동일하게 판정된다.
- 근거는 "Randomness creates variation, not causation"이다. 등급이 전투 수학을 건드리면 결과를 매치업으로 설명할 수 없게 된다.
- **기준 등급은 중립이어야 한다.** 밸런스 측정이 그 등급에서 이루어지므로, 중립이 아니면 측정값이 어떤 등급도 설명하지 못한다. 검증기가 강제한다.
- 정보 난이도는 사실을 지어내지 않는다. 해석자 수준·탐색 범위·Evidence 강도를 바꿔 추정 범위와 편향만 움직이며 참값은 그대로다. 더 좋은 Analyst를 고용한 것과 같은 종류의 변화다.
- 쉬운 등급은 무능한 해석자에게 바닥을 깔아주되 유능한 해석자를 깎지 않는다.

---

## 콘텐츠·화면·접근성·현지화·성능

선행 조건이 충족된 항목을 유예에서 해제했다.

| 항목 | 세부 기준 |
|---|---|
| 콘텐츠 물량 목표 | `docs/design/35_content_volume_targets.md` |
| 화면별 정보구조 | `docs/design/36_screen_information_architecture.md` |
| 접근성·현지화·성능 | `docs/design/37_accessibility_localization_performance.md` |

- **콘텐츠 목표는 개수가 아니라 균형 흡수량이다.** "몇 개를 만들었나"가 아니라 "만든 것 중 몇 개가 최적 플레이에 등장하나"로 잰다. 개수 목표는 죽은 카드를 쌓아도 달성된다.
- **죽은 카드 판정에는 카드 종류에 맞는 질문을 쓴다.** 고정 계획 균형(`calibrate solve`)은 한 경기 내내 같은 콤보를 반복한다고 가정하므로, 이동·회피·각처럼 **읽고 나서 쓰는 카드**를 구조적으로 과소평가한다. 조건부 카드는 반응형 정책 균형(`calibrate policies`)의 **타임라인 사용률**로 판단한다. 어느 쪽이든 **수렴 전(외부 최적대응 이득 0.2 초과) 해의 목록은 읽지 않는다.**
- **화면은 정보구조만 정하고 와이어프레임은 두지 않는다.** 모든 화면은 `FighterKnowledgeState`를 통해 보며 `FighterTrueState`를 직접 읽지 않는다. 능력치를 보여주는 곳에는 반드시 범위와 신뢰도가 함께 간다. 숫자 하나만 보여주면 "해석은 틀릴 수 있다"가 거짓이 된다.
- 낮은 신뢰도를 UI가 보정해주지 않는다. 능력 낮은 해석자가 좁은 범위를 자신 있게 제시하면 화면도 그대로 보여준다. 나쁜 Scout는 위험해야 한다.
- **난이도와 접근성은 다른 축이다.** 난이도는 도전의 크기, 접근성은 조작·인지 부담이다. 접근성 옵션이 게임을 쉽게 만들지 않는다.
- **현지화는 사용자 노출 문자열만 외부화한다.** 검증 오류 메시지는 설계 문서의 검증 기준과 1:1 대응하므로 원문을 유지한다. 콘텐츠 문자열은 이미 Config에 있으므로 언어 레이어를 더하는 일이지 코드를 고치는 일이 아니다.
- **성능은 측정한 축에만 예산을 둔다.** Config 로드 100ms, 교환 판정 2ms, JS 400KB. 프레임률·타깃 기기·발열은 **실기기 QA 전이므로 목표를 세우지 않는다.** 측정 없이 숫자를 적으면 근거 없는 목표가 된다.

## 아직 선행 조건이 충족되지 않은 것

| 항목 | 남은 선행 조건 |
|---|---|
| 목표 프레임률·타깃 기기·발열·배터리 | 실기기 QA. 현재 측정값이 없다 |

이 하나를 제외하면 §34의 유예 목록은 비었다.

---|---|
| UI 화면별 정보구조·와이어프레임 | Milestone A 이후. 재미가 검증되기 전 화면을 굳히면 시스템이 화면에 끌려간다 |
| 콘텐츠 물량 목표 (Action / Technique / Skill Card 개수) | 엔진 검증 이후. 로드맵 19장의 "많은 콘텐츠로 엔진 문제를 가리지 않는다"를 따른다 |
| 접근성 옵션 | UI 확정 이후. 난이도가 도전의 크기라면 접근성은 조작·인지 부담이며 축이 다르다 |
| 현지화 정책 | 텍스트 물량이 생긴 뒤 |
| 성능 목표와 타깃 기기 | 실기기 QA 이후. 현재는 측정값이 없다 |

이 목록을 비워두지 않고 명시하는 이유는, 누락과 유예를 구분하기 위해서다.
...
- Three.js 입체 렌더러는 도트 전환으로 대체되었고 **배포 경로에서 제외했다.** 해당 스택과 vendor 717KB는 `archive/three-renderer/`로 옮겨 삭제하지 않고 보관한다. 도달하지 않는 코드를 모든 사용자에게 전송하지 않으면서 비례 시스템과 IK의 재사용 가치는 남긴다. 기존 테스트는 그대로 유지된다.
